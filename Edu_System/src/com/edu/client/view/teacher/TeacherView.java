...
public class TeacherView {
	...
	JPanel teacherInformationPanel, teacherCoursePanel,
			teacherTestResultsPanel, teacherCommentPanel, modifyPasswordPanel;
	...
	JComboBox teacherTestResultYearJC, teacherTestResultTermJC,
			teacherTestResultCourseNameJC, teacherTestResultCourseCharacterJC;// 依次是学年、学期、课程名称、课程性质JComboBox;
	 ...
	 * 添加教师评价的各个组件
	 */
	JComboBox teacherCommentYearJCBox, teacherCommentTermJCBox,
			teacherCommentCourseNameJCBox;
	JTextField teacherCommentStatisticsText, teacherCommentLevelText,
			teacherCommentForTeacherText, teacherCommentGradeTermText;
	DefaultTableModel teacherCommentItmizeStatisticsDefaultTable,
	...
	JTextField modifyPasswordUserNameText, modifyPasswordUserIdText,
			modifyPasswordOldPasswordText, modifyPasswordNewPasswordText,
			modifyPasswordEnsureNewPasswordText;
	JButton modifyPasswordEnsureButton;

	public void init(Teacher teacher, Vector<String> teacherCourseVeGradeJCBox,
			Vector<String> teacherCourseVeMajorJCBox,
			Vector<String> teacherVeYearJCBox,
			Vector<String> teacherVeTermJCBox,
			Vector<String> teacherVeCourseNameJCBox,
			Vector<String> teacherTestResultVeCourseCharacterJCBox) {
		 ...
		 */
		teacherTabbedPane = new JTabbedPane(JTabbedPane.LEFT,
		 ...
		 * 设置个人信息的标签,以及修改、确定按钮
		 */
		JLabel teacherNumLabel = new JLabel("编号：" + teacher.getTeacherId());
		teacherNumLabel.setFont(font);
		teacherNumLabel.setBounds(xWieht / 2 - 100, hight + 20, 500, 20);

		JLabel teacherNameLabel = new JLabel("姓名:" + teacher.getTeacherName());
		teacherNameLabel.setFont(font);
		teacherNameLabel.setBounds(xWieht / 2 - 100, hight + 60 + hightAdd,
				500, 20);

		JLabel teacherPhoneLabel = new JLabel("电话号码:"
				+ teacher.getTeacherPhone());
		teacherPhoneLabel.setFont(font);
		teacherPhoneLabel.setBounds(xWieht / 2 - 100, hight + 100 + 2
				* hightAdd, 500, 20);

		JLabel teacherAddressLabel = new JLabel("家庭地址:"
				+ teacher.getTeacherAddress());
		teacherAddressLabel.setFont(font);
		teacherAddressLabel.setBounds(xWieht / 2 - 100, hight + 140 + 3
				* hightAdd, 500, 20);

		JLabel teacherPositionLabel = new JLabel("职称:"
				+ teacher.getTeacherTitle());
		teacherPositionLabel.setFont(font);
		teacherPositionLabel.setBounds(xWieht / 2 - 100, hight + 180 + 4
				* hightAdd, 500, 20);

		JLabel teacherCollegeLabel = new JLabel("学院:" + teacher.getTeaColId());
		teacherCollegeLabel.setFont(font);
		teacherCollegeLabel.setBounds(xWieht / 2 - 100, hight + 220 + 5
				* hightAdd, 500, 20);

		teacherReviseButton = new JButton("修改");
		teacherReviseButton.setBounds(xWieht / 2 + 80, hight + 220 + 10
				* hightAdd, 60, 20);
		teacherSureButton = new JButton("确定");
		teacherSureButton.setBounds(xWieht / 2 + 180, hight + 220 + 10
				* hightAdd, 60, 20);
		...
		for (int i = 0; i < 5; i++) {
			for (int j = 0; j < 9; j++) {
				...
				teacherCourseTabels[i][j]
						.setBackground(new Color(208, 216, 232));
